...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is IntegerType:
            ...
            return len(inType.dimensions)*"[" + self.getJVMType(inType.typ)
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) in [cgen.ClassType, StringType, cgen.ArrayPointerType, ArrayType]:
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitINSTANCEFIELD(self, lexeme, in_, frame):
        # lexeme: String
        # in_: Type
    ...
    *   @param in the type of the operands.
    ...
    def emitNOT(self, in_, frame):
        ...
        result = list()
        result.append(self.emitIFTRUE(FALSELABEL, frame))
    ...
    *   @param in the type of the operands.
    ...
    def emitMOD(self, frame):
        ...
        # Override machinecode definition
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    '''   generate code to initialize a glocal array variable.<p>
    *   @param index the index of the local variable.
    *   @param in the type of the local array variable.
    '''

    def emitInitNewArray(self, name, size, eleType, frame, init = None, isNotStatic = None):
        # name = var name, size = dimension, eleType = atomicType, frame = frame, init = code of init
        result = []
        size = list(map(lambda x: int(x), size))
        if len(size) == 1:
            # It's a single dimensian array
            size = size[0]
            ...
            if init is not None:
                ...
                for idx in range(size):
                    ...
                    result.append(init[idx])
                    result.append(self.emitASTORE(eleType, frame))
                    if idx < size-1:
                        result.append(self.emitDUP(frame))
            if isNotStatic is None:
                result.append(self.jvm.emitPUTSTATIC(name, self.getJVMType(cgen.ArrayPointerType(eleType))))
            else: result.append(self.emitWRITEVAR(name, ArrayType(size, eleType), isNotStatic, frame))
        else:
            # This is multi-dimensions array
            for dimen in size:
                result.append(self.emitPUSHICONST(dimen, frame))
            result.append(self.jvm.emitMULTIANEWARRAY(self.getJVMType(ArrayType(size, eleType)), str(len(size))))
            if isNotStatic is None:
                result.append(self.jvm.emitPUTSTATIC(name, self.getJVMType(ArrayType(size, eleType))))
            else: result.append(self.emitWRITEVAR(name, ArrayType(size, eleType), isNotStatic, frame))
    ...
    def emitCloneArray(self, addressIndex, eleType, frame):
        result = []
        result.append(self.emitREADVAR(
            "", cgen.ArrayPointerType(eleType), addressIndex, frame))
        result.append(JasminCode.INDENT + "invokevirtual " + "[" + self.getJVMType(
            eleType) + "/clone()Ljava/lang/Object;" + JasminCode.END)
        result.append(JasminCode.INDENT +
                      "checkcast [" + self.getJVMType(eleType) + JasminCode.END)
    ...
    *   .source MT22.CLASSNAME.java<p>
    *   .class public MT22.CLASSNAME<p>
